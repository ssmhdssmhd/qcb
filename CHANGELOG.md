...
### 修复 clean.php 默认返回播放器页面而非 m3u8 内容的问题

#### 问题分析

之前的逻辑会在浏览器直接访问时显示播放器页面，但用户希望 **默认返回 m3u8 内容**，这样播放器才能直接使用这个链接进行播放。

#### 修复内容

**简化 shouldShowPlayerPage() 判断逻辑**
- 默认返回 m3u8 内容（Content-Type: application/vnd.apple.mpegurl）
- 只有明确指定 `player=1` 参数时才显示播放器页面

**访问方式**
- `clean.php?id=xxx` → 返回 m3u8 内容（播放器可直接使用）
- `clean.php?id=xxx&player=1` → 显示播放器页面（手动浏览）
