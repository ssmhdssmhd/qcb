...
**本次更新重新设计了顶部蓝色区域的布局，删除了冗余的后台管理入口，新增从远程 `gg.txt` 获取公告的功能，整体更加简洁美观。**

**1. 顶部蓝色区域布局重构** ([mxadmin.php](file:///workspace/mxadmin.php))
- 🗑️ **删除后台管理入口**：移除右侧冗余的后台管理卡片，布局更简洁
- 📢 **新增公告区域**：V2 接口下方新增公告卡片，从远程 `gg.txt` 动态获取
- 📐 **单列布局**：改为上下结构，V2 接口占满整行，视觉更舒展
- 🎯 **信息聚焦**：突出 V2 统一接口的核心地位，减少视觉干扰

**2. 远程公告功能** ([mxadmin.php](file:///workspace/mxadmin.php))
- 🌐 **远程获取**：从同目录下的 `gg.txt` 文件动态加载公告内容
- 📅 **日期识别**：支持 `[YYYY-MM-DD] 公告内容` 格式，自动解析并显示日期
- 🔴 **新公告标记**：第一条公告自动标记为最新（红色圆点 + 发光效果）
- ⏳ **加载状态**：加载中显示闪烁动画，失败时显示友好提示
- 📜 **滚动区域**：公告内容可滚动查看，自定义细滚动条样式
- 🔄 **自动缓存刷新**：URL 带时间戳，确保获取最新公告

**3. 公告文件格式说明** ([gg.txt](file:///workspace/gg.txt))
```
[2026-07-14] v4.3.0 版本发布：新增公告功能
[2026-07-14] 系统维护通知：今晚22:00-23:00例行维护
普通公告（不带日期）
```

## [4.2.0] - 2026-07-14

### ✨ UI 优化：顶部接口区域全新设计 + 布局体验优化

**本次更新对管理后台顶部蓝色接口预览区域进行了全面的视觉升级，采用现代化的玻璃拟态设计，提升整体用户体验。**

**1. 顶部接口预览区域全新设计** ([mxadmin.php](file:///workspace/mxadmin.php))
- ✨ **玻璃拟态风格**：所有卡片采用 `backdrop-filter: blur()` 毛玻璃效果，更具现代感
- 🎨 **层次化布局**：V2 统一接口与后台管理采用双列网格布局，信息层次更清晰
- 🔽 **自定义下拉框**：重新设计类型选择器，带有自定义箭头图标和交互动画
- 📋 **优化复制按钮**：更大的点击区域、图标+文字、悬停上浮阴影效果
- 💫 **装饰光晕效果**：添加两个径向渐变光晕，增强视觉深度
- 🌊 **波浪过渡**：底部波浪形过渡装饰，与下方内容区域自然衔接
- 📱 **完善响应式**：移动端自动切换单列布局，适配各种屏幕尺寸
...
**本次更新实现了深度去广告功能，针对直接压缩在视频中的广告和插播内容进行深度检测和移除。**

**1. 新增 `mxjx/deep` 深度去广告分析接口** ([mx.php](file:///workspace/mx.php))
- ✅ **三重过滤**：M3U8标签过滤 + 规则引擎过滤 + TS片段MD5内容分析
- ✅ **TS MD5内容分析**：下载TS片段头部（8KB），计算MD5哈希值，识别重复出现的广告片段
- ✅ **广告时间段返回**：返回所有广告的时间段信息（start/end/duration/source），供前端播放器使用
- ✅ **广告特征码自学习**：检测到的广告MD5自动保存到数据库，下次播放时直接识别
- ✅ **时间段合并**：将规则过滤和TS分析检测到的广告时间段合并、排序、去重

**2. `mxjx` 接口支持深度去广告模式** ([mx.php](file:///workspace/mx.php))
- ✅ 新增 `deep=1` 参数，启用TS片段MD5内容分析
- ✅ 在规则过滤后，对保留的片段进行TS MD5分析，识别嵌入广告
- ✅ 通过 `X-Deep-Ad-Removed` 和 `X-Deep-Ad-Segments` 响应头反馈深度去广告统计

**3. 前端播放器广告时间段自动跳过** ([player/index.php](file:///workspace/player/index.php))
- ✅ 播放时异步请求 `mxjx/deep` 接口获取广告时间段
- ✅ 每500ms检查播放进度，当进入广告时间段时自动跳过
- ✅ 跳过广告时显示提示信息
- ✅ 切换原始地址时清除广告跳过逻辑

**4. 优化TS片段移除性能** ([mx.php](file:///workspace/mx.php))
- ✅ 优化m3u8片段移除逻辑，使用索引遍历替代array_search，性能提升显著
...
**问题原因**:
- 腾讯视频 `getinfo` API 返回的清晰度名称（如 "shd"、"hd"）被误识别为视频标题
- 导致后续更准确的 `float_vinfo2` API 不会被调用
- 标题只有 "hd" 两个字符，无法正确搜索资源

**修复内容** ([OfficialReplaceManager.php](file:///workspace/gz/OfficialReplaceManager.php) / [DbOfficialReplaceManager.php](file:///workspace/db/DbOfficialReplaceManager.php)):

**1. 优化腾讯视频 API 调用顺序**
- ✅ 将 `float_vinfo2` API 移到最前面（返回标题最准确）
- ✅ 增加标题最小长度校验（至少3个字符才视为有效标题）
- ✅ 移除冗余的单独 float_vinfo2 调用分支，统一到循环中

**2. 新增无效标题过滤（findTitleInData）**
- ✅ 过滤清晰度相关关键词：hd, shd, fhd, 4k, 8k, 标清, 高清, 超清, 蓝光, 1080p, 720p, sd, md, ld, 流畅, 准高清
- ✅ 过滤其他无效标题：vip, 免费, 预告, 花絮
- ✅ 短标题（<6字符）包含无效关键词时直接过滤

**3. 优化标题解析（parseVideoTitle）**
- ✅ 新增 `_数字` 格式集数识别（如 "遮天_02" → 第2集）
- ✅ 新增 `-数字` 格式集数识别（如 "遮天-02" → 第2集）
- ✅ 调整解析顺序：先解析明确的季数 → 再解析集数 → 最后处理末尾数字
- ✅ 末尾数字无明确标识时，默认识别为集数而非季数（更符合常见命名习惯）

**修复效果**:
| 测试标题 | 修复前 | 修复后 |
|---------|--------|--------|
| 遮天_02 | 基础:遮天, 季:2 | 基础:遮天, 集:2 ✅ |
| 遮天02 | 基础:遮天, 季:2 | 基础:遮天, 集:2 ✅ |
| 遮天 第二季 | 基础:遮天, 季:2 | 基础:遮天, 季:2 ✅ (不变) |
| 庆余年 第二季 第3集 | 季:2, 集:3 | 季:2, 集:3 ✅ (不变) |
| 三体S01E05 | 季:1, 集:5 | 季:1, 集:5 ✅ (不变) |
...
**分析历史修复**:
- ✅ **analyzeVideo 添加历史记录** - 视频分析完成后自动保存到历史
- ✅ **aiSkipVideo 添加历史记录** - AI去广告完成后自动保存到历史
- ✅ **saveToHistory 数据结构适配** - 支持 analyze/ai_skip/batch 多种返回格式
- ✅ **批量分析进度修复** - 进度计算从 `i/length` 修正为 `(i+1)/length`
- ✅ **批量分析历史记录** - 批量分析的每条结果都独立保存到历史

**仪表盘统计修复**:
- ✅ **MD5卡片改为AI去广告** - 统计卡片内容与实际数据一致
- ✅ **统计趋势动态更新** - 今日新增、成功率等趋势数据实时计算
- ✅ **平均耗时计算优化** - 基于成功分析的平均耗时更准确
- ✅ **热门域名排行功能** - 新增Top 10热门域名排行，带进度条和排名样式
- ✅ **Dashboard切换刷新** - 切换到仪表盘时重新渲染所有统计

**历史记录页面**:
- ✅ **筛选和搜索正常** - 按类型筛选、按域名/URL搜索
- ✅ **重新分析跳转** - 点击历史记录可跳转对应页面并填充URL
- ✅ **复制URL功能** - 一键复制历史记录的视频地址
- ✅ **清空历史确认** - 清空历史前二次确认

**代码优化**:
- ✅ **合并DOMContentLoaded** - 统一入口初始化，避免重复注册
- ✅ **函数模块化** - renderTopDomains 等功能独立封装
...
**背景图功能**:
- 📥 **内置 20 张高质量背景图** - 存储于 `img/bj/` 目录
- 🎲 **随机切换** - 每次刷新或点击切换按钮随机更换
- 🪟 **毛玻璃效果** - 卡片、侧边栏、顶部栏全部 backdrop-filter 毛玻璃
- 🌈 **渐变色彩叠加** - 蓝紫粉三色渐变缓慢流动，自动变色
- ✨ **平滑过渡** - 背景图切换 0.8s 淡入淡出

**控制按钮**（顶部栏右侧）:
- 🖼️ **背景图开关** - 一键开启/关闭背景图模式
- 🔄 **换一张** - 随机切换下一张背景图
- 💾 **记忆功能** - 自动记住上次的背景图和开关状态

**毛玻璃适配组件**:
- ✅ 卡片 (card) - 85% 透明度 + 12px 模糊
- ✅ 侧边栏 (sidebar) - 90% 透明度 + 16px 模糊
- ✅ 顶部栏 (header) - 85% 透明度 + 12px 模糊
- ✅ 数据统计卡片 (stat-card) - 85% 透明度 + 12px 模糊
- ✅ 快捷操作卡片 (quick-action) - 80% 透明度 + 8px 模糊
- ✅ 移动端底部导航 - 90% 透明度 + 16px 模糊
- ✅ 输入框 / 下拉框 - 90% 透明度 + 8px 模糊

**自动变色动画**:
- 🎨 **渐变流动动画** - 蓝→紫→粉 三色渐变 15s 循环流动
- 💫 **视觉层次丰富** - 背景图 + 渐变叠加 + 毛玻璃 = 通透高级感
- 🌙 **深色模式适配** - 背景图模式下深色模式同样毛玻璃效果

---

## [3.0.0] - 2026-07-11

### 🎨 全新 UI 设计（PlainAdmin 风格）

**设计语言重构**:
- ✨ **极简现代风格** - 对标 PlainAdmin / TailAdmin 设计语言
- 🎨 **品牌蓝渐变主色调** - #3b82f6 → #8b5cf6 渐变，视觉更高级
- 📐 **8px 间距系统** - 统一间距规范，布局更协调
- 🔵 **大尺寸圆角卡片** - 16px 圆角，柔和阴影，现代感十足
- 🌈 **CSS 变量系统** - 完整的设计 token，支持深色模式
- 🎭 **丰富的色彩系统** - 主色/成功/警告/危险/信息/紫色/粉色 7 套语义色
...
**功能模块**:
- ⚡ 快捷操作区 - 6 个常用功能入口卡片
- 🕐 最近分析记录 - 实时显示最近 5 条分析历史
- 🔥 热门域名排行 - 展示分析次数最多的域名

### 📜 新增分析历史记录

**核心功能**:
- 💾 自动保存分析记录（最多 100 条，localStorage 本地存储）
- 🔍 支持按 URL/域名搜索过滤
- 📂 支持按类型筛选（全部/视频分析/AI去广告/MD5分析）
- 🔄 一键重新分析，点击历史记录快速跳转
- 📋 一键复制 URL
- 🗑️ 支持清空历史记录

**记录信息**:
- 分析类型图标（AI去广告/MD5分析/视频分析）
- 域名与完整 URL
- 分析时间（友好显示：刚刚/X分钟前/X小时前）
- 成功/失败状态徽章
- 总片段数与广告片段数
...
**移动端专属优化**:
- 🍔 汉堡菜单按钮 + 抽屉式侧边栏
- 🧭 底部导航栏（首页/AI去广告/分析/规则/历史）
- 👆 触控优化：按钮 ≥ 44px，输入框 ≥ 44px
- 📱 安全区域适配（iPhone 刘海屏）
- 🔄 侧滑遮罩层，点击空白关闭
...
**菜单重构**:
- 新增「工作台」分组：数据概览、分析历史、批量分析
- 菜单按使用频率重新排序，AI去广告标 HOT 标签
- 菜单图标 + 文字 + 徽章，信息层级清晰

**性能优化**:
- CSS 变量驱动，主题切换 0 延迟
- 页面切换 0.3s 缓动动画，流畅自然
- 所有图片资源使用系统 emoji，零额外请求
- localStorage 本地存储历史，不占用服务器资源

### 📦 兼容性

- ✅ 保留所有原有功能，100% 向下兼容
- ✅ 支持 7 套主题色切换（默认紫/金/绿/蓝/青/红/深色）
- ✅ 桌面端 / 平板端 / 手机端 全适配
- ✅ 所有原有 API 接口不受影响
...
### ⚡ MD5 分析性能大幅优化（速度提升 5-10 倍）

**核心优化**:
- 🚀 **curl_multi 并发下载** - 默认 15 并发，极速模式 20 并发，并行下载 TS 片段
- 📦 **大幅减少下载量** - 从 1MB → 16KB → 8KB（极速模式），速度提升 60 倍以上
- ⏱️ **更短超时设置** - 连接超时 1s，总超时 2-3s，快速跳过慢请求
- 🎯 **智能采样策略** - 极速模式仅分析 25 个片段，兼顾速度与准确率
- 🔗 **TCP_NODELAY + keep-alive** - 连接复用，减少握手开销
- ⚡ **Range 请求优化 - 支持 206 部分内容响应，验证数据完整性
- 🧠 **更快的 select 等待 - 极速模式 0.1s 轮询，普通模式 0.2s

**新增 API 参数**:
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `fast` | 极速模式开关 | `1` (开启) |
| `sample` | 采样模式 (auto/head/even/both) | `auto` |
| `count` | 最大分析片段数 | 极速30 / 普通60 |
| `concurrency` | 并发下载数 | 极速20 / 普通15 |

### 🎨 前端体验优化

**AI 自动去广告页面**:
- ✅ 优化极速模式提示文案，显示预期分析时间 2-5 秒
- ✅ 优化加载状态展示：并发下载 + 智能采样 + 仅下载文件头

**AI 插播识别页面**:
- ✅ 新增极速模式支持（aiInsertFastMode）
- ✅ 优化加载状态展示
- ✅ 显示极速模式标识
...
**TsMd5Analyzer 新增方法**:
- `setFastMode($enabled)` - 一键开启极速模式（20并发 + 8KB下载 + 1s连接超时 + 2s总超时 + 25片段采样
- `setTimeout($connect, $total)` - 自定义超时时间
- 优化 `multiFetchMd5()` - 支持 HTTP 206 部分内容验证，支持下载大小校验
- 优化 `createCurlHandle()` - 添加 TCP_NODELAY、gzip 压缩支持、动态 buffer 缓冲区
