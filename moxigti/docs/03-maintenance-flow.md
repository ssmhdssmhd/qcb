...
sequenceDiagram
    ...
    participant State as 状态层 (state.js)
    participant Core as 核心框架 (VersionManager)
    participant File as .version.json

    Dev->>CLI: node src/index.js bump "修订说明"
    CLI->>State: loadManager()
    State->>File: 读取持久化状态
...
- 每次功能新增 / Bug 修复 / 文档维护，都执行一次 `node src/index.js bump "说明"`。
- 可使用 `node src/index.js history` 随时回看版本演进历史。
