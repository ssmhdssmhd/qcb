...
// ===== 整体重构 v5.14.0：解析核心迁移到 handlers/ 模块 =====
// 加载 handlers（解析核心 action 均由此分发，mx.php 仅做路由 + 管理类 action）
if (file_exists($rootDir . '/handlers/autoload.php')) {
    require_once $rootDir . '/handlers/autoload.php';
}

// 加载 parse/ 框架（资源站优先 + 全局预算 + 归一化输出）
$parseLoaded = false;
if (file_exists($rootDir . '/parse/autoload.php')) {
    require_once $rootDir . '/parse/autoload.php';
    $parseLoaded = true;
}

// 构建 handler 上下文（依赖注入，避免 handler 直接依赖 mx.php 内部变量）
$handlersCtx = new HandlersContext();
$handlersCtx->rootDir = $rootDir;
$handlersCtx->useDb = !empty($useDb);
$handlersCtx->ruleManager = $ruleManager ?? null;
$handlersCtx->siteManager = $siteManager ?? null;
$handlersCtx->officialReplaceMgr = $officialReplaceMgr ?? null;
$handlersCtx->officialMgr = $officialMgr ?? null;
$handlersCtx->proxyManager = $proxyManager ?? null;
$handlersCtx->updateManager = $updateManager ?? null;
$handlersCtx->parseLoaded = $parseLoaded;
...
try {
    // ===== 整体重构 v5.14.0：解析核心 action 优先走 handlers 分发 =====
    // 命中（moxi/mxjx/mxjx/info/mxjx/deep/xiami_jx/skip/parse…）则由 handler 输出并退出；
    // 未命中继续走下方管理类 case。
    if (HandlerDispatcher::dispatch($action, $handlersCtx)) {
        exit;
    }
    ...
    switch ($action) {
        ...
        // ===== 解析核心已迁移至 handlers/HandlerDispatcher（整体重构 v5.14.0）=====
        // case 'skip' / 'mxjx' / 'mxjx/info' / 'mxjx/deep' / 'xiami_jx' / 'xiami_jx/info'
        //   / 'moxi' / 'moxi/api' / 'parse' / 'parse/parse' / 'jx' / 'parse/info' / 'jx/info'
        // 均由 switch 之前的 HandlerDispatcher::dispatch() 处理，此处为安全回退兜底。
        case 'update/version':
            $versionFile = '';
