...
CREATE TABLE IF NOT EXISTS `domain_rules` (
    ...
    `duration_rules` JSON COMMENT '时长规则数组',
    `discontinuity_rules` JSON COMMENT 'DISCON规则数组',
    `sequence_jump_rules` JSON COMMENT '序列号跳跃规则数组',
    `filename_patterns` JSON COMMENT '文件名模式数组',
    `insertion_patterns` JSON COMMENT '插播模式数据',
    `ad_type_stats` JSON COMMENT '广告类型统计',
    `psychological_profile` JSON COMMENT '心理画像数据',
    `marker_stats` JSON COMMENT '标记统计数据',
    `analysis_stats` JSON COMMENT '分析统计概览',
    `history_stats` MEDIUMTEXT COMMENT '历史学习数据(JSON, 大字段)',
    `marker_detection` JSON COMMENT '标记检测配置',
    `confidence` JSON COMMENT '置信度配置',
