...
class Database {
    ...
    private function loadConfig() {
        ...
        if (file_exists($configFile)) {
            ...
            if (is_array($config)) {
                // 如果配置要求 MySQL，但本地 MySQL 明显不可达，自动降级到 SQLite
                // （零配置部署友好：首次部署用户不会被 mysql 连接拒绝卡住）
                if (($config['type'] ?? 'sqlite') === 'mysql') {
                    $host = $config['mysql_host'] ?? '127.0.0.1';
                    $port = $config['mysql_port'] ?? 3306;
                    $conn = @fsockopen($host, $port, $errno, $errstr, 2);
                    if (!$conn) {
                        $config['type'] = 'sqlite';
                        // 如果 db_config 是我们自动写的，就改写它；用户手写的保留避免覆盖
                        $autoGenerated = @file_get_contents($configFile);
                        if (strpos($autoGenerated, '首次运行自动生成') !== false
                            || strpos($autoGenerated, 'auto-generated') !== false
                            || strpos($autoGenerated, 'fallback_from_mysql') !== false) {
                            $config['fallback_from_mysql'] = true;
                            $config['fallback_reason'] = "MySQL $host:$port unreachable";
                            $content = "<?php\n// 数据库配置文件（首次运行自动生成，可按实际环境修改）\n"
                                . "// 本文件由脚本从 MySQL fallback 到 SQLite（MySQL 不可达自动降级）\n"
        ...
        // 自动从 example 文件创建 db_config.php（第一次部署时零配置）
        $exampleFile = __DIR__ . '/db_config.php.example';
        $defaultConfig = [
            'type' => 'sqlite',  // 即使 example 写了 mysql，我们也默认先从 SQLite 起步（零配置最稳）
        ...
        if (file_exists($exampleFile)) {
            $exampleConfig = @require $exampleFile;
            if (is_array($exampleConfig)) {
                // 保留 example 里的 host/port/账号 信息，但强制起始 type=sqlite
                $defaultConfig = array_replace($defaultConfig, $exampleConfig);
                $defaultConfig['type'] = 'sqlite';
                $defaultConfig['_example_merged'] = true;
            }
        }

        // 持久化默认配置为 db_config.php，方便后续修改
        if (is_writable(__DIR__)) {
            $content = "<?php\n// 数据库配置文件（首次运行自动生成，可按实际环境修改）\n"
                . "// 如需使用 MySQL，把 'type' 改为 mysql，并填写下方 mysql_* 账号密码重启\n"
                . "// 生成时间: " . date('Y-m-d H:i:s') . "\nreturn "
                . var_export($defaultConfig, true) . ";\n";
            @file_put_contents($configFile, $content);
        }
