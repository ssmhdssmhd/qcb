...
- 🚀 **修复 jiexi.php 不能同时调用官解和官替**
  - 新增 `getVideoLinkByConcurrentRace()` 函数：把所有已启用的官解接口 + 官替接口合并到同一个 curl_multi 并发池
  - 真正的多线程高并发：同时发起多个 HTTP 请求，最快成功的立即返回
  - 自动识别通道（official/replace），后续按通道分流处理
  - 总耗时 ≈ 最快的那个接口的耗时，而非多个接口耗时之和
- ⚡ **新增配置项 `performance.concurrent_race_enabled`（默认 `true`）**
  - 开启：同时调用官解和官替，速度最快
  - 关闭：回到旧的串行 fallback 逻辑（向后兼容）
- 💡 **并发模式强制启用官替**
  - 即使后台「嗅探设置」中 `replace_api.enabled = false`，并发模式也会自动用本地官替接口
  - 确保两条通道同时跑，真正实现"同时调用"
- 🧵 **`max_concurrent` 自动扩展**
  - 并发模式下，max_concurrent 至少覆盖官解+官替总数，避免某通道被排到剩余队列串行调用
