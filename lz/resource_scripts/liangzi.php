 ...
 * 量子资源站去广告脚本
 * 资源站：量子
 * 官网：http://23.224.101.30
 * 采集接口：https://cj.lziapi.com/api.php/provide/vod/from/lzm3u8/
 * 
 * 调用方式：http://你的域名/lz/resource_scripts/liangzi.php?url=视频链接
 */
...
function sendLiangziJson($data, $code = 200) {
...
function resolveMasterPlaylistLiangzi($url) {
...
try {
    switch ($action) {
        case 'search':
            ...
            if (empty($wd)) {
                sendLiangziJson(['code' => 400, 'msg' => '缺少 wd 参数', 'data' => null], 400);
            }
            ...
            if ($response === false || $httpCode != 200) {
                sendLiangziJson(['code' => 500, 'msg' => '搜索请求失败', 'data' => null], 500);
            }
        ...
        default:
            ...
            $mediaUrl = resolveMasterPlaylistLiangzi($url);
            if ($mediaUrl !== $url) {
