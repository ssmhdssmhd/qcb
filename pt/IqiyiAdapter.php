...
class IqiyiAdapter extends AbstractPlatformAdapter
{
    ...
    public function extractVideoId($url)
    {
        ...
        if (preg_match('/\/v_([a-zA-Z0-9_]+)\.html?/i', $url, $matches)) {
            ...
            // ?vfrm=xxx 格式
            $videoId = $matches[1];
        } elseif (preg_match('/\/([a-zA-Z0-9]{10,24})\.html?$/i', $url, $matches)) {
            // 纯 ID 形式的 .html
            $videoId = $matches[1];
    ...
    protected function parseEpisodeInfo($data)
    {
        ...
        foreach ($sources as $src) {
            if ($info['total_episodes'] === null) {
                foreach (['total', 'episode_count', 'video_count', 'count'] as $k) {
                    if (isset($src[$k])) {
            ...
            if ($info['episode_num'] === null) {
                foreach (['order', 'episode', 'index'] as $k) {
                    if (isset($src[$k])) {
            ...
            if ($info['episode_name'] === '') {
                foreach (['episode_name', 'name', 'title', 'short_title'] as $k) {
